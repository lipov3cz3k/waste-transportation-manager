...
INNER JOIN Address
	ON Address.id = Track.start_id
INNER JOIN OSMLocation
	ON OSMLocation.obj_id = Address.location_id
WHERE
	((OSMLocation.latitude BETWEEN 40 AND 50) AND (OSMLocation.longitude BETWEEN 10 AND 20)) OR
	((Address.latitude BETWEEN 40 AND 50) AND (Address.longitude BETWEEN 10 AND 20))
GROUP BY x.path_id